<template>
  <div class="max-w-5xl mx-auto py-10 px-4">
    <h2 class="text-2xl font-bold mb-6 text-[#3871B1]">{{ $t('admin.analytics.title') }}</h2>
    <div class="grid md:grid-cols-3 gap-4">
      <StatBox :label="$t('admin.analytics.revenue')" :value="`$${revenue}`" />
      <StatBox :label="$t('admin.analytics.creditsSold')" :value="creditsSold" />
      <StatBox :label="$t('admin.analytics.creditsUsed')" :value="creditsUsed" />
    </div>
    <div class="grid md:grid-cols-2 gap-4 mt-6">
      <StatBox :label="$t('admin.analytics.lowCreditUsers')" :value="lowCreditCount" />
      <StatBox :label="$t('admin.analytics.topTeacher')" :value="topTeacher" />
    </div>
  </div>
</template>
